"""Unified configuration for AnkiBlitz Core.
...
``save_section`` so the tools stay decoupled. A new feature just adds another
section to ``DEFAULTS`` and a tab to the settings dialog — nothing else changes.

This is the Core build: the auto-reveal (``speed_focus``) and progressive-reveal
(``word_reveal``) sections live in the separate aSFM and Progressive Word Reveal
add-ons, so they're absent here.
"""
